...
freq_threshold = 1 # Changed this to 1 because our data set is small
...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout, glove_embeddings=None):
        super(Encoder, self).__init__()

        # Initialize the embedding layer with pre-trained GloVe embeddings
        self.embedding = nn.Embedding(input_dim, emb_dim)
        
        # If glove embeddings are provided, set the weights of the embedding layer
...
enc_emb_dim = 300 # Changed from 256
dec_emb_dim = 300 # Changed from 256
enc_hid_dim = 512
dec_hid_dim = 512
dropout = 0.5
...
glove_embeddings = load_glove_embeddings(glove_file, input_vocab, embedding_dim=300)

# Initialize Encoder and Decoder
encoder = Encoder(input_dim, enc_emb_dim, enc_hid_dim, dec_hid_dim, dropout, glove_embeddings=glove_embeddings).to(device)
